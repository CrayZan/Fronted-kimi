...
export default function Booking() {
  ...
  const [formData, setFormData] = useState({
    checkIn: '', checkOut: '', roomType: '', guests: '1', name: '', email: '', phone: '',
  });
    ...
    try {
      const response = await fetch("https://hostal-milagro.onrender.com/create_preference", {
        ...
        body: JSON.stringify({
          ...formData,
          amount: deposit,
          title: `Reserva - ${formData.roomType}`
        }),
      });
      ...
      toast.error('Error de conexión. Revisa si el servidor de Render está despierto.');
    } finally {
      setLoading(false);
    }
  };
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <Input placeholder="+54..." required value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: e.target.value })} />
                </div>
              </div>
              {totalPrice > 0 && (
                <div className="bg-orange-50 p-4 rounded-lg border border-orange-200">
                  ...
                  <p className="text-xs text-orange-600 mt-2 flex items-center gap-1"><Info className="w-3 h-3"/> Redirigiendo a Mercado Pago...</p>
                </div>
              )}
